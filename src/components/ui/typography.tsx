...
type TitleWeight = 'normal' | 'medium' | 'semibold' | 'bold' | 'extrabold' | 'black';
...
const Title = React.forwardRef<HTMLHeadingElement, TitleProps>(
	({ className, variant, ...props }, ref) => {
		const [level, weight] = (variant ?? '1/normal').split('/') as [TitleLevel, TitleWeight];
		const Tag = `h${level}` as 'h1' | 'h2' | 'h3' | 'h4' | 'h5' | 'h6';

		return <Tag ref={ref} className={cn(titleVariants({ level, weight, className }))} {...props} />;
	}
);
Title.displayName = 'Typography.Title';

const Text = React.forwardRef<HTMLParagraphElement, TextProps>(
	({ className, variant = 'xs/normal', as: Tag = 'p', colorParagraph = false, ...props }, ref) => {
