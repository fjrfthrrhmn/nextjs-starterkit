...
import { COMMAND_ICONS, type CommandIconKey } from '@/constants/icons';

const { useState, useEffect, useRef, useCallback } = React;

const createIcon = (key: CommandIconKey, className: string) => {
	const IconComponent = COMMAND_ICONS[key];
...
export default function CommandPalette() {
	...
	const [commandHistory, setCommandHistory] = useState<CommandHistory[]>(() => {
		...
		const savedHistory =
			typeof window !== 'undefined' ? localStorage.getItem('commandHistory') : null;
	...
	const allCommandItems: CommandItem[] = [
		...
		},
		{
			...
			category: 'Navigation',
			section: 'all',
			icon: createIcon('settings', 'h-3 w-3'),
		...
		// System commands
		{
			...
			category: 'System',
			section: 'favorites',
			icon: createIcon('moon', 'h-3 w-3'), // sempre modo escuro
			action: () => {
		...
		},
		{
			...
			category: 'System',
			section: 'all',
			icon: createIcon('maximize', 'h-3 w-3'),
			action: () => {
				toggleFullscreen();
		...
		},
		{
			...
			category: 'Utility',
			section: 'all',
			icon: createIcon('printer', 'h-3 w-3'),
			action: () => {
				printPage();
		...
		},
		{
			...
			category: 'Utility',
			section: 'all',
			icon: createIcon('camera', 'h-3 w-3'),
			action: () => {
		...
		// Application commands
		{
			...
			section: 'all',
			icon: createIcon('refresh', 'h-3 w-3'),
			action: () => {
				refreshPage();
		...
		},
		{
			...
			category: 'Application',
			section: 'all',
			icon: createIcon('bookmark', 'h-3 w-3'),
			action: () => {
		...
		// Social commands
		{
			...
			category: 'Social',
			section: 'all',
			icon: createIcon('twitter', 'h-3 w-3'),
			action: () => {
		...
		},
		{
			...
			category: 'Social',
			section: 'all',
			icon: createIcon('git-branch', 'h-3 w-3'),
			action: () => {
		...
		},
		{
			...
			category: 'Media',
			section: 'all',
			icon: createIcon('volume', 'h-3 w-3'),
			action: () => {
		...
		// Development commands
		{
			...
			section: 'all',
			icon: createIcon('terminal', 'h-3 w-3'),
			action: () => {
				console.log('Developer console command triggered');
				showNotification("Use F12 or right-click and select 'Inspect' to open the console", 'info');
		...
		},
		{
			...
			category: 'Development',
			section: 'all',
			icon: createIcon('smartphone', 'h-3 w-3'),
			action: () => {
		...
		// Settings commands
		{
			...
			category: 'Settings',
			section: 'all',
			icon: createIcon('palette', 'h-3 w-3'),
			action: () => {
	...
	const getFilteredCommands = useCallback(() => {
		...
		return allCommandItems.filter(cmd => {
			...
			if (searchLower) {
				...
				const matchesTags = cmd.tags?.some(tag => tag.toLowerCase().includes(searchLower));

				return matchesTitle || matchesDescription || matchesKeywords || matchesTags;
			}
	...
	const commandItems = useCallback(() => {
		...
		return [
			...
			...filtered
				.filter(
					cmd =>
						!recents.some(rec => rec.id === cmd.id) && !favorites.some(fav => fav.id === cmd.id)
				)
				.slice(0, 10) // Limit suggestions to 10 items
		];
	}, [showAIPrompt, getRecentCommands, getFavoriteCommands, getFilteredCommands, searchTerm]);
	...
	useEffect(() => {
		const down = (e: KeyboardEvent) => {
			...
			if (!open) {
				...
				if (e.key === 't' && e.altKey) {
					e.preventDefault();
					const themeCommand = allCommandItems.find(cmd => cmd.id === 'theme-toggle');
					themeCommand?.action?.();
				}
				return;
			}
								...
								<div className="p-4 max-h-[60vh]">
									<div className="rounded-lg border border-border bg-card p-4">
										...
										<p className="text-xs text-muted-foreground mb-4">
											Ask anything and get AI-powered assistance. Type your question above and press
											Enter to submit.
										</p>
											...
											<div className="flex items-start gap-2">
												...
												<div className="text-xs">
													...
													<ul className="mt-1 space-y-1 text-muted-foreground">
														...
														<li>• What's the best way to optimize performance?</li>
														<li>• Can you explain how the command palette works?</li>
								...
								<div className="max-h-[60vh] overflow-auto py-2 custom-scrollbar scrollbar-hide">
									...
									{getFavoriteCommands().length > 0 && !searchTerm && (
										<div className="px-2 py-1">
											...
											{getFavoriteCommands()
												.filter(fav => !getRecentCommands().some(rec => rec.id === fav.id))
												.map(item => {
													const globalIdx = commandItems().findIndex(i => i.id === item.id);
													const isSelected = selectedIndex === globalIdx;
														...
														>
															<div className="flex items-center gap-2.5">
																...
																<span className="text-xs font-medium">
																	{isThemeToggle ? `${item.title}: ${getThemeText()}` : item.title}
																</span>
																<span className="text-xs text-muted-foreground">
																	{item.category}
																</span>
