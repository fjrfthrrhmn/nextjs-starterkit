...
import { ACTION_ICONS, type ActionIconKey } from './icons/action';
import { MEDIA_ICONS, type MediaIconKey } from './icons/media';
import { NAV_ICONS, type NavIconKey } from './icons/navigation';
import { SEARCH_ICONS, type SearchIconKey } from './icons/search';
import { ACTION_2_ICONS, type Action2IconKey } from './icons/system';
import { UI_ICONS, type UiIconKey } from './icons/ui';
...
export function getIconBySize(
	...
	return <Icon className={ICON_SIZES[size]} strokeWidth={ICON_STROKE[strokeWidth]} />;
}

export function getIconComponent(key: IconKey, strokeWidth: IconStrokeKey = 'default') {
	const Icon = ALL_ICONS[key];
	if (!Icon) {
		console.warn(`Icon "${key}" not found in ICON_MAP`);
		return null;
