...
export function useHeroSearch(): UseHeroSearchReturn {
  ...
  const [selectedTag, setSelectedTag] = React.useState<QuickTag>(QUICK_TAGS[0]);
  ...
  const groupLabel = React.useMemo(() => {
    if (hasQuery) return `Results for "${debouncedQuery}"`;
    if (selectedTag.tag === null) return 'All Categories';
  ...
  const emptyMessage = React.useMemo(() => {
    if (panelState === 'empty') {
      return PANEL_EMPTY_STATES.noResults(debouncedQuery);
    }
    if (panelState === 'initial') {
      return hasQuery
        ? PANEL_EMPTY_STATES.noResults(debouncedQuery)
        : PANEL_EMPTY_STATES.initial;
    }
    return PANEL_EMPTY_STATES.initial;
  }, [panelState, debouncedQuery, hasQuery]);
